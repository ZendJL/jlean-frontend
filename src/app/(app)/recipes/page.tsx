...
export default function RecipesPage() {
  ...
  if (view === 'detail' && active) {
    // La API devuelve: macrosTotal, macrosPerServing, items[]
    ...
    return (
      <div className="space-y-6 max-w-3xl">
        ...
        <div className="bg-surface rounded-xl border border-border px-4 py-3 grid grid-cols-4 gap-4 text-center">
          {[
            { label: 'Calories', val: `${total.calories} kcal` },
              ...
              {items.map((ing: any) => (
                <li key={ing.id} className="px-4 py-2.5 flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-medium text-text">{ing.macros?.calories ?? 0} kcal</p>
                    <p className="text-xs text-text-faint">
                      P {ing.macros?.protein ?? 0}g
                      {' · '} C {ing.macros?.carbs ?? 0}g
                      {' · '} F {ing.macros?.fat ?? 0}g
                    </p>
                  </div>
                </li>
              ))}
      ...
      {showNew && (
        <div className="bg-surface rounded-xl border border-border p-4 space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowNew(false)} className="text-text-muted text-sm hover:text-text">
              Cancel
            </button>
          </div>
        </div>
      ...
      {(!recipes || recipes.length === 0) ? (
        <div className="bg-surface rounded-xl border border-border p-8 text-center">
          ...
          <p className="text-text-muted text-sm mt-1">
            Create your first recipe to reuse it in your daily log.
          </p>
        </div>
      ) : (
